...
// --- Custom NCNN Layers (Imported from main.cpp) ---
...
Java_com_agusibrahim_fastalpr_FastAlpr_init(JNIEnv* env, jobject thiz, jobject assetManager) {
    ...
    if (g_ocr_net->load_param(mgr, "ocr.param") != 0 ||
        g_ocr_net->load_model(mgr, "ocr_patched.bin") != 0) {
        LOGE("[-] Failed to load OCR model from Assets!");
        delete g_yolo_net; g_yolo_net = nullptr;
...
Java_com_agusibrahim_fastalpr_FastAlpr_nativeDetect(JNIEnv* env, jobject thiz, jobject bitmap, jboolean doOcr) {
    pthread_mutex_lock(&g_lock);
    if (!g_yolo_net || (doOcr && !g_ocr_net)) {
        LOGE("[-] Native libraries not initialized!");
        pthread_mutex_unlock(&g_lock);
        return nullptr;
    }

    // 1. Get Bitmap Info & Lock Pixels
    ...
    // Convert Android Bitmap RGBA to clean RGB unsigned char buffer
    std::vector<unsigned char> img_rgb(img_w * img_h * 3);
    unsigned char* p_rgb = img_rgb.data();
    const uint32_t* p_rgba = (const uint32_t*)pixels;
    for (int i = 0; i < img_w * img_h; ++i) {
        uint32_t c = p_rgba[i];
        p_rgb[i * 3 + 0] = (c & 0x000000FF);       // R
        p_rgb[i * 3 + 1] = (c & 0x0000FF00) >> 8;  // G
        p_rgb[i * 3 + 2] = (c & 0x00FF0000) >> 16; // B
    }
    AndroidBitmap_unlockPixels(env, bitmap);
    ...
    if (yolo_ex.extract("/end2end/NonMaxSuppression_output_0", nms_out) != 0 ||
        ...
        LOGE("[-] Failed to extract YOLOv9 NMS output");
        pthread_mutex_unlock(&g_lock);
        return nullptr;
    }
    ...
    LOGI("[+] YOLOv9 found %d plates", (int)kept_bboxes.size());
    ...
    for (size_t d = 0; d < kept_bboxes.size(); ++d) {
        ...
        if (doOcr) {
            ...
            // Shape (w=3, h=128, c=64) expected by NCNN OCR
            ...
            if (ocr_ex.extract("plate", ocr_out) == 0) {
                ...
                det.ocr_conf = ocr_scores.empty() ? 0.0f : (sum / ocr_scores.size());
                LOGI("  OCR Output [%d]: %s (Score: %.2f)", (int)d, ocr_text.c_str(), det.ocr_conf);
            }
        }
        detections.push_back(det);
    }

    // 5. Convert C++ Detections to Java Array of AlprResult objects
    jclass result_cls = env->FindClass("com/agusibrahim/fastalpr/AlprResult");
    jmethodID result_ctor = env->GetMethodID(result_cls, "<init>", "(IIIIFLjava/lang/String;F)V");

    jobjectArray result_array = env->NewObjectArray(detections.size(), result_cls, nullptr);
    for (size_t i = 0; i < detections.size(); ++i) {
        const auto& det = detections[i];
        jstring text_jstr = det.text.empty() ? nullptr : env->NewStringUTF(det.text.c_str());
        
        jobject result_obj = env->NewObject(result_cls, result_ctor,
                                            det.bbox.x1,
                                            det.bbox.y1,
                                            det.bbox.width(),
                                            det.bbox.height(),
        ...
        if (text_jstr) env->DeleteLocalRef(text_jstr);
